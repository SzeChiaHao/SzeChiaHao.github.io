...
顶部导航的栏目由 `src/consts.ts` 里的 `CATEGORIES` 数组控制：

```ts
export const CATEGORIES = ['代数', '分析', '几何', '数论', '组合', '初等'];
```

- 文章 frontmatter 的 `tags` 里含有某个栏目名，该文章就会自动归入该栏目（对应 `/tags/栏目名/` 页面）。
- **增删栏目**：直接增删上面数组里的字符串即可，导航和栏目页会自动更新。
- 标签页对**所有** tag 都生成（不只栏目），所以文章里的任意 tag 都能点。

## 文章元信息与目录

每篇文章会自动显示：作者（`frontmatter` 里的 `author`，缺省用 `consts.ts` 的 `AUTHOR`）、发布时间、预计阅读时长、标签；并依据文章里的 `##`/`###` 标题自动生成右侧目录（TOC，随滚动高亮当前小节）。
...
- 明暗主题切换 + 响应式
- RSS 订阅（`/rss.xml`）
- 文章右侧目录 TOC（随滚动高亮）、作者/时间/阅读时长/标签元信息
